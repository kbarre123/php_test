...
//This shows how to connect to DB, query and generate a chart using mysqli
...
$Result  = mysqli_query($db,
	"SELECT * FROM Avg_Prices WHERE product_id=1"
	);
...
while($row = mysqli_fetch_array($Result)) {
	$week_id[]   = $row["week_print"];
	$avg_price[] = $row["avg_price"];
}
 
/* Save the data in the pData array */
$myData->addPoints($week_id,"week_print"); //don't change name
$myData->addPoints($avg_price,"Average Price"); //name used in the legend

/* Put the week_print column on the abscissa axis */
$myData->setAbscissa("week_print");
